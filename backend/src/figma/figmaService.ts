...
export class FigmaService {
  ...
  private async runConvert(input: ConvertFigmaDto) {
    ...
    const { fileKey, nodeId } = this.parseFigmaUrl(input.figmaUrl)
    const figmaData = await this.fetchFigmaNode(fileKey, nodeId, input.token.trim())
    ...
    const simplifiedDesign = await extractFigmaAsJSON(
      figmaData as Parameters<typeof extractFigmaAsJSON>[0],
      {
        fileKey,
  ...
  // 从 figma url 中提取 fileKey 与 nodeId，集中校验逻辑
  private parseFigmaUrl(figmaUrl: string): { fileKey: string; nodeId: string } {
  ...
  // 调用 Figma 节点接口，并兼容 { body } 包裹返回结构
  private async fetchFigmaNode(fileKey: string, nodeId: string, token: string): Promise<unknown> {
